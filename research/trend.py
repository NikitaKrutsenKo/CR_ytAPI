...
from research.domain import CollectionBundle, Status, utc, iso
...
class TrendConfig:
    ...
    def __post_init__(self):
        if any(not math.isfinite(v) or v <= 0 for v in (self.window_hours, self.half_life_hours, self.slope_floor, self.max_gap_windows)):
        ...
        if not 1 <= self.baseline_min_windows <= self.baseline_limit or self.min_support <= 0 or self.min_creators < 1:
...
class TrendEnricher:
    ...
    def ingest(self, bundle: CollectionBundle) -> None:
        ...
        if bundle.metadata.status == Status.COMPLETE and not bundle.metadata.truncated:
            self.coverage.append((utc(bundle.metadata.effective_from).timestamp(), utc(bundle.metadata.effective_to).timestamp()))

    def window(self, end, hours):
        lower = end.timestamp()-hours*3600
        return [v for v in self.identities.values() if lower <= utc(v.published_at).timestamp() < end.timestamp()]
    ...
    def prune(self, end, hours=48):
        lower = end.timestamp()-hours*3600
        self.identities = {k: v for k, v in self.identities.items() if utc(v.published_at).timestamp() >= lower}
...
class TrendEngine:
    ...
    def engagement(self, deltas: list[CounterDelta]) -> float | None:
        ...
        for delta in deltas:
            if delta.like_rate is None or delta.comment_rate is None or delta.correction:
                continue
            ql = percentile(delta.like_rate, self.like_history)
            qc = percentile(delta.comment_rate, self.comment_history)
            if ql is not None and qc is not None:
                values.append(.7*ql+.3*qc)
        # Normalize every member of this observation against the same prior cohort.
        self.like_history.extend(d.like_rate for d in deltas if d.like_rate is not None and not d.correction)
        self.comment_history.extend(d.comment_rate for d in deltas if d.comment_rate is not None and not d.correction)
        self.like_history = self.like_history[-1000:]
    ...
    def process(self, bundle: CollectionBundle) -> dict | None:
        ...
        n_eff = float(origins)
        valid = (bundle.metadata.status == Status.COMPLETE and not bundle.metadata.truncated
        ...
        row = {"timestamp": timestamp, "topic": bundle.topic.canonical_name, "batch_id": bundle.metadata.batch_id,
               "formula_version": config.formula_version, "signal_basis": "youtube_query_publication_activity",
               "youtube_activity_count": len(items) if valid else None,
               "sampled_video_count": len(items), "youtube_activity_rate": len(items)/config.window_hours if valid else None,
               "unique_creators": creators, "effective_sample_size": n_eff, "ewma": None,
               "velocity": None, "acceleration": None, "growth": None, "burst": None,
               "engagement": None, "breadth": None, "confidence_N": min(n_eff/50, 1),
               "confidence_D": .5*min(origins/10, 1)+.5/3,
               "freshness": 1.0 if valid else (2**(-hours/config.half_life_hours) if state.ewma is not None else 0.0),
               "completeness": (state.valid_windows+int(valid))/state.expected_windows,
               "confidence_O": None, "confidence": None, "source_cap": config.source_cap,
               "trend_score": None, "youtube_research_score": None, "lifecycle": state.lifecycle,
               "score_status": "UNAVAILABLE: production source panel absent", "research_gate": "INSUFFICIENT_EVIDENCE"}
        if not valid:
        ...
        contiguous = previous_time is not None and hours <= config.max_gap_windows*config.window_hours and state.velocity is not None
        smoothed = rate if state.ewma is None else rate*(1-2**(-hours/config.half_life_hours))+state.ewma*2**(-hours/config.half_life_hours)
        velocity = (math.log1p(smoothed)-math.log1p(state.ewma))/hours if previous_time and state.ewma is not None and hours <= config.max_gap_windows*config.window_hours else None
        ...
        growth = min(max(velocity or 0, 0)/max(quantile(positive, .9) if positive else 0, config.slope_floor), 1) if enough and velocity is not None else None
        burst = None
        if enough:
            logs = [math.log1p(r["ewma"]) for r in prior]
            center = median(logs)
            scale = max(1.4826*median(abs(v-center) for v in logs), .25)
        ...
        confidence = min(.3*row["confidence_N"]+.2*row["confidence_D"]+.2*row["freshness"]+.2*row["completeness"]+.1*duration, config.source_cap)
        row.update(ewma=smoothed, velocity=velocity, acceleration=acceleration, growth=growth, burst=burst,
                   engagement=engagement, breadth=breadth, confidence_O=duration, confidence=confidence)
        gate = (growth is not None and burst is not None and breadth is not None and n_eff >= config.min_support
                and origins >= config.min_creators and state.valid_windows >= 3 and confidence >= .45 and row["completeness"] >= .8)
        if gate:
            # Separate named formula versions avoid silently renormalizing production weights.
            score = 100*((.45*growth+.25*burst+.15*engagement+.10*breadth)/.95 if engagement is not None
                         else (.55*growth+.30*burst+.15*breadth))
            row["youtube_research_score"] = score
            row["research_gate"] = "PASS_WITH_ENGAGEMENT" if engagement is not None else "PASS_NO_ENGAGEMENT_v1"
