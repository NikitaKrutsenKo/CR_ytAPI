...
class TrendConfig:
    ...
    Attributes:
        formula_version: Version identifier string for formula configuration.
...
class TrendState:
    ...
    Attributes:
        ...
        lifecycle: Inferred lifecycle state ('WATCHING', 'RISING', 'BREAKOUT', 'PEAK', 'COOLING').
...
class TrendEngine:
    """Deterministic YouTube Trend Engine implementing mathematical components: EWMA, G, Z, B, E.
    ...
    Mathematical Lineage:
        ...
        2. Velocity:
           v_t = (ln(1 + r_t) - ln(1 + r_(t-1))) / dt

        3. Acceleration:
           a_t = (v_t - v_(t-1)) / dt

        4. Growth (G):
           G = clip( max(v_t, 0) / max(Q_0.90(prior_pos_v), slope_floor), 0.0, 1.0 )

        5. Burst Factor (Z):
           logs = [ln(1 + r) for r in prior_ewma]
           center = median(logs)
           scale = max(1.4826 * median(|logs - center|), 0.25)
           z = (ln(1 + r_t) - center) / scale
           Z = clip( max(z, 0) / 4.0, 0.0, 1.0 )

        6. Breadth (B_YT):
           B_YT = ECDF_prior(creators)

        7. Engagement (E_YT):
           E_YT = mean( 0.7 * ECDF_prior(Δlikes/Δviews) + 0.3 * ECDF_prior(Δcomments/Δviews) )

        8. Confidence:
           N = min(n_eff / 50, 1)
           D = 0.5 * min(origins / 10, 1) + 0.5 / 3
           F = 2 ** (-dt / H)
           M = valid_windows / expected_windows
           O = min(valid_duration / 3, 1)
           confidence = min( 0.3*N + 0.2*D + 0.2*F + 0.2*M + 0.1*O, source_cap )

        9. YouTube Research Score:
           With engagement:
           score = 100 * (0.45*G + 0.25*Z + 0.15*E_YT + 0.10*B_YT) / 0.95

           Without engagement:
           score = 100 * (0.55*G + 0.30*Z + 0.15*B_YT)
    """
    ...
    def engagement(self, deltas: list[CounterDelta]) -> float | None:
        ...
        Formula:
            E_i = 0.7 * ECDF_prior(like_rate_i) + 0.3 * ECDF_prior(comment_rate_i)
            E_YT = mean(E_i)
        """
        values = []
        for delta in deltas:
    ...
    def _components(self, rate: float, hours: float, previous_time, creators: int) -> tuple:
        """Compute derivatives, growth, burst, and breadth against prior state only.

        Formulas:
            EWMA: smoothed = rate if first else rate * (1 - 2^(-dt/H)) + prev_ewma * 2^(-dt/H)
            Velocity: (ln(1 + smoothed) - ln(1 + prev_ewma)) / hours
            Acceleration: (velocity - prev_velocity) / hours
            Growth G: clip( max(v, 0) / max(Q_0.90(prior_pos_v), slope_floor), 0, 1 )
            Burst Z: clip( max((ln(1 + smoothed) - center) / scale, 0) / 4, 0, 1 )
            Breadth B: ECDF_prior(creators)
        """
        config, state = self.config, self.state
        contiguous = (
    ...
    def _publish(self, row: dict, origins: int, n_eff: float) -> None:
        """Evaluate evidence gates and update heuristic lifecycle state."""
        config, state = self.config, self.state
