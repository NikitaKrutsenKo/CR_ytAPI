"""Experiment configuration input form and window preview."""
...
def number(value, low=0.01, high=100000, integer=False):
    """Helper to create a configured QSpinBox or QDoubleSpinBox."""
...
class ExperimentForm(QWidget):
    ...
    def __init__(self, root: Path, profiles) -> None:
        super().__init__()
    ...
    def choose_pool(self, name: str) -> None:
        """Populate topic text box when a preset category pool is selected."""
        pools = {
    ...
    def update_mode(self) -> None:
        """Lock window mode to STATIC for historical validation."""
    ...
    def update_window(self) -> None:
        """Refresh window preview when window mode changes."""
        self.start.setEnabled(True)
        self.end.setEnabled(True)
        self.update_preview()

    def update_preview(self) -> None:
        """Recompute and render the human-readable effective publication window preview."""
        try:
    ...
    def config(self) -> ExperimentConfig:
        """Construct and validate an ExperimentConfig from active form inputs."""
        requests = []
    ...
    def load_config(self, config: ExperimentConfig) -> None:
        """Populate form inputs from an existing ExperimentConfig instance."""
