...
from youtube.client import YouTubeApiError, RequestCancelled
...
def observation(item: dict, timestamp: str) -> VideoObservation:
    ...
    counters = [int(stats[k]) if stats.get(k) is not None else None for k in ("viewCount", "likeCount", "commentCount")]
    if any(v is not None and v < 0 for v in counters):
        raise ValueError("Negative cumulative counter")
    return VideoObservation(VideoIdentity(item["id"], snippet["channelId"], published, snippet.get("title", "")), timestamp, *counters)
    ...
    def collect(self, request: CollectionRequest, last_success=None, now=None) -> CollectionBundle:
        ...
        try:
            for _ in range(request.max_pages):
                requested += 1
                page = self.search.page(request.topic.canonical_name, request.page_size, request.order,
                                        iso(effective), iso(end), token)
                pages += 1
                for item in page.get("items", []):
                    received += 1
                    try:
                        snippet = item["snippet"]
                        identity = VideoIdentity(item["id"]["videoId"], snippet["channelId"], iso(utc(snippet["publishedAt"])), snippet.get("title", ""))
                        identities.setdefault(identity.video_id, identity)
                    except (KeyError, ValueError, TypeError):
                        warnings.append("Malformed discovery item omitted")
                        status = Status.PARTIAL
                log.info("page_collected topic_id=%s page=%s items=%s", request.topic.topic_id, pages, len(page.get("items", [])))
            ...
            warnings.append(str(exc))
        finished = now_utc()
        metadata = CollectionMetadata(uuid4().hex, iso(now), request.topic.canonical_name,
            iso(start), iso(end), iso(effective), iso(end), request.window_mode, request.rolling_hours,
            request.page_size, request.max_pages, requested, pages, received, len(identities),
            max(0, received-len(identities)-sum(w.startswith("Malformed discovery") for w in warnings)),
            self.client.profile, iso(now), iso(finished), (time.monotonic()-started)*1000, status,
            truncated=truncated, config_hash=fingerprint(request), aliases=request.topic.aliases, order=request.order)
        log.info("collection_completed batch_id=%s topic_id=%s status=%s unique=%s", metadata.batch_id, request.topic.topic_id, status, len(identities))
        return CollectionBundle(request.topic, metadata, tuple(observations), tuple(identities.values()),
                                tuple(self.client.telemetry[telemetry_start:]), tuple(warnings))
