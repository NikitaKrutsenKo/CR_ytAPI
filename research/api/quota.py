"""Quota management, SQLite persistent daily ledger, and preflight estimations."""
...
from research.core.domain import ExperimentConfig, Mode
from research.core.time import now_utc


class QuotaStopped(RuntimeError):
    """Execution was blocked by local quota safety limits before making an API call."""


@dataclass(frozen=True)
class QuotaEstimate:
    """Preflight estimation of daily API calls and quota costs for an experiment.

    Attributes:
        search_calls: Worst-case estimated search calls.
        other_units: Worst-case estimated read units (videos, channels, playlistItems).
        status: Risk rating: 'SAFE', 'WARNING', or 'WOULD EXCEED BUDGET'.
        label: Human-readable description label.
        endless: True if experiment is running in endless mode.
        search_calls_per_hour: Estimated search rate per hour.
        other_units_per_hour: Estimated other read rate per hour.
        search_calls_per_pacific_day: Projected search calls per Pacific day.
        other_units_per_pacific_day: Projected other read units per Pacific day.
    """
...
class QuotaManager:
    """Atomic daily per-profile quota ledger using SQLite.

    Maintains separate daily budget pools for search calls (100 units/call) and
    other read operations (videos, channels, playlistItems). Quota resets at midnight
    Pacific time (America/Los_Angeles), matching Google YouTube API accounting.
    """
        ...
        with closing(sqlite3.connect(self.path)) as conn:
            with conn:
                conn.execute(
                    ...
                    "profile TEXT, day TEXT, endpoint TEXT, calls INTEGER, "
                    "PRIMARY KEY(profile,day,endpoint))"
                )

    def _day(self) -> str:
        """Return the current Pacific calendar date string (YYYY-MM-DD)."""
    ...
    def usage(self) -> dict:
        ...
        with closing(sqlite3.connect(self.path)) as conn:
            rows = dict(
                conn.execute(
                    "SELECT endpoint,calls FROM usage WHERE profile=? AND day=?",
                    (self.profile, self._day()),
                )
            )
        return {
    ...
    def next_reset(self) -> datetime:
        """Calculate the UTC datetime of the next Pacific midnight quota reset."""
    ...
    def seconds_until_reset(self) -> float:
        """Return seconds remaining until the next Pacific midnight reset."""
        return max(0.0, (self.next_reset() - self.clock()).total_seconds())

    def consume(self, endpoint: str) -> None:
        """Pre-charge the ledger atomically before making a request.

        Raises:
            QuotaStopped: If the call would exceed the safety-budgeted quota.
        """
    ...
    def estimate(self, config: ExperimentConfig) -> QuotaEstimate:
        """Estimate worst-case quota consumption for an experiment plan."""
        config.validate()
