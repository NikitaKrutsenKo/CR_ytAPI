...
class TopicScheduler:
    ...
    def __init__(self, topics: tuple[CandidateTopic, ...], exploration_fraction=.3):
    ...
    def choose(self, costs: dict[str, float]) -> TopicSelection:
        ...
        def priority(topic):
            ...
            score = (topic.priority+self.signals[identifier]+uncertainty+.1*waiting)/max(costs[identifier], 1)
        ...
        return TopicSelection(identifier, "exploration" if exploration else "active_monitoring",
                              score, self.signals[identifier], uncertainty, costs[identifier])
