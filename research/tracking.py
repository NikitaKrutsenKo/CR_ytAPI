...
from research.domain import VideoObservation, utc, encode
    ...
    def observe(self, observations: tuple[VideoObservation, ...]) -> list[CounterDelta]:
        ...
        for current in observations:
            ...
            if previous:
                ...
                if hours <= 0:
                    continue
                changes = [None if a is None or b is None else a-b for a, b in
                           zip((current.views, current.likes, current.comments), (previous.views, previous.likes, previous.comments))]
                ...
                results.append(CounterDelta(identifier, current.timestamp, hours, *changes, *rates,
                    likes/views if eligible and likes is not None else None,
                    comments/views if eligible and comments is not None else None, correction))
            self.latest[identifier] = current
        # Deterministic recency selection prevents unbounded growth in long runs.
        ranked = sorted(self.latest.values(), key=lambda v: (v.timestamp, v.views or 0, v.identity.video_id), reverse=True)
