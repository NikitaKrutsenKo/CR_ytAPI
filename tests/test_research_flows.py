...
from research.trend import TrendEngine, TrendConfig, percentile
from research.evaluation import EventEvaluator, HistoricalAnalyzer
from research.scheduling import TopicScheduler
from tests.test_research_ingestion import FakeClient, NOW, video
...
@pytest.mark.parametrize("mode,has_gap,has_trend", [(Mode.GAP, True, False), (Mode.TREND, False, True), (Mode.COMBINED, True, True)])
def test_live_modes_shared_discovery_and_offline_replay(tmp_path, mode, has_gap, has_trend):
    ...
    def factory(*args, **kwargs):
        ...
        return client
    config = ExperimentConfig(mode, (CollectionRequest(CandidateTopic.named("AI")),), duration_minutes=.001)
    manager = ExperimentManager(tmp_path, ApiProfiles(tmp_path/".env", {"YOUTUBE_API_KEY_DEFAULT": "fake"}), factory)
...
def test_counter_deltas_missing_corrections_and_repetitions():
    ...
    assert row.delta_views == 200 and row.views_per_hour == 200 and row.like_rate == .05
...
def make_trend_bundle(hour=12, count=30, status=Status.COMPLETE):
    ...
    identities = tuple(VideoIdentity(f"v{hour}_{i}", f"c{i}", iso(end-timedelta(minutes=30))) for i in range(count))
    return replace(bundle, metadata=replace(bundle.metadata, timestamp=iso(end), effective_from=iso(end-timedelta(hours=24)), effective_to=iso(end), status=status), discovery=identities)
...
def test_historical_and_events_are_evaluation_only():
    ...
    event = KnownEvent(bundle.topic.topic_id, "Synthetic fixture", "control", "2026-09-17T12:00:00Z", "test", "https://example.com/fixture", "synthetic", "2026-09-17T12:00:00Z")
...
def test_scheduler_deterministic_exploration():
    ...
    first, second = TopicScheduler(topics, .3), TopicScheduler(topics, .3)
